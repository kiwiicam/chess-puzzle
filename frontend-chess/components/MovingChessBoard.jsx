    ...
    <GestureHandlerRootView>
      <View style={styles.wrapper} pointerEvents="none">
        ...
        <View style={styles.bottom}>
          {files.map((f, index) => <Text key={`bottom-${index}`} style={[styles.label, { paddingTop: 4 }]}>{f}</Text>)}
        </View>
        <View style={styles.left}>
          {ranks.map((r, index) => <Text key={`left-${index}`} style={[styles.label, { paddingRight: 4 }]}>{r}</Text>)}
        </View>
        ...
        <View style={styles.board}>
          <Chessboard
            fen={FEN}
            boardSize={300}
            colors={{ black: "#5b7789", white: "#c2dce2", lastMoveHighlight: 'rgba(255,255,0, 0.0)' }}
            ref={chessboardRef}
            durations={{ move: 500 }}
            withLetters={false}
            withNumbers={false}
